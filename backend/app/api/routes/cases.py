...
@router.get("/{case_id}/fax-confirmation")
async def get_fax_confirmation(case_id: str, db: AsyncSession = Depends(get_db)):
    """Serve the Fax Confirmation PDF generated by the RC fax-status poller.

    The PDF is produced when case.fax_status transitions to 'Sent' and
    uploaded to Azure Blob at case.fax_confirmation_pdf_key.

    404 if the case doesn't have a confirmation yet (fax still Queued,
    or failed).
    """
    case = await repo.get_case(db, case_id)
    if not case:
    ...
    if not case.fax_confirmation_pdf_key:
        raise HTTPException(
            status_code=404,
            detail="No fax confirmation available yet for this case.",
        )

    from app.ingest.blob_fetcher import _get_container

    blob_key = case.fax_confirmation_pdf_key
    try:
        container = _get_container()
        blob_client = container.get_blob_client(blob_key)
        data = blob_client.download_blob().readall()
    except Exception as e:
        logger.error(f"Failed to fetch fax confirmation for case {case_id}: {e}")
        raise HTTPException(status_code=502, detail=f"Blob fetch failed: {e}")

    filename = f"fax-confirmation-{case.exam_id or case_id}.pdf"
    return Response(
        content=data,
        media_type="application/pdf",
        headers={"Content-Disposition": f"inline; filename=\"{filename}\""},
...
def _serialize_case(case) -> dict:
    return {
        ...
        "fax_status": getattr(case, "fax_status", None),
        "fax_delivered_at": case.fax_delivered_at.isoformat()
            if getattr(case, "fax_delivered_at", None) else None,
        "fax_confirmation_pdf_key": getattr(case, "fax_confirmation_pdf_key", None),
    }
